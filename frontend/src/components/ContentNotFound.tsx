...
const CONTENT = {
  ...
  shared: {
    ...
    subtext: 'Consultations can be shared and unshared anytime by their creators.',
    primaryCta: { label: 'Start Your Own Consultation', to: '/cases/new' },
    secondaryCta: { label: 'Go Home', to: '/' },
  },
} as const;

export function ContentNotFound({ variant, isAuthenticated = false }: ContentNotFoundProps) {
  const content = CONTENT[variant];

  // For case variant, swap CTAs based on auth status
  const primaryCta = variant === 'case' && !isAuthenticated
    ? content.secondaryCta
    : content.primaryCta;

  const secondaryCta = variant === 'case' && !isAuthenticated
    ? { label: 'Go Home', to: '/' }
    : content.secondaryCta;
